...
class KashfRaqabiScreen extends StatefulWidget {
  const KashfRaqabiScreen({super.key});

  @override
  State<KashfRaqabiScreen> createState() => _KashfRaqabiScreenState();
}

class _KashfRaqabiScreenState extends State<KashfRaqabiScreen> {
  // Form controllers and variables
  String? selectedMohafaza;
  int? selectedMohafazaId; // إضافة ID المحافظة المحددة
  String? selectedAidiya;
  String? selectedStation;
  int? selectedStationId; // إضافة ID المحطة المحددة
  ...
  Future<void> _fetchStationsFromServer() async {
    print('=== _fetchStationsFromServer called ===');

    if (selectedMohafazaId == null || selectedAidiya == null) {
      return;
    }
    ...
    try {
      final requestBody = {
        'CityId': selectedMohafazaId.toString(),
        'OwnerShip': selectedAidiya!,
      };

      print('Request body: $requestBody');

      // ================== التصحيح النهائي هنا ==================
      // The header key has been corrected based on your Postman screenshot.
      final response = await http.post(
        Uri.parse('https://enviroment.technounityopdc.com/api/DetectionAPI/GetStationsByCityAndOwnership'),
        headers: {
          "Hdr-GetStations": 'Stations-2025-OK',
        },
        body: requestBody,
      );
      // =======================================================
      ...
      if (response.statusCode == 200) {
        ...
        List<dynamic> stationsJson = responseData as List;

        setState(() {
          gasStations = stationsJson.map((stationJson) => GasStation.fromJson(stationJson)).toList();
          stations = gasStations.map((station) => station.gasStationName).toList();
          for (var station in gasStations) {
            stationsMap[station.gasStationName] = station.id;
          }
        });
      } else {
      ...
      print('Exception caught in _fetchStationsFromServer: $e');
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              'حدث خطأ في تحميل المحطات: ${e.toString()}',
...
// Function to submit form with all data including the image
  Future<void> _submitForm() async {
    ...
    try {
      ...
      var uri = Uri.parse('https://enviroment.technounityopdc.com/api/DetectionAPI/Create');
      var request = http.MultipartRequest('POST', uri);

      // 2. إضافة الهيدر المطلوب
      request.headers['Hdr-Create'] = 'Create-2025-OK';

      // 3. تحديث أسماء الحقول لتطابق Postman
      request.fields['CityId'] = selectedMohafazaId.toString();
      request.fields['GasStationId'] = selectedStationId.toString(); // تم التعديل
      request.fields['Ownership'] = selectedAidiya!;                // تم التعديل
      request.fields['DetectionDate'] = selectedDate!.toIso8601String();
        ...
        return Directionality(
          ...
          child: AlertDialog(
            ...
            content: Column(
              ...
              children: [
                ListTile(
                  leading: Icon(Icons.camera_alt, color: const Color(0xFF53A13D), size: 24.r),
                  title: Text(
                    'التقط صورة',
                    style: GoogleFonts.cairo(),
                  ),
                ...
                ListTile(
                  leading: Icon(Icons.photo_library, color: const Color(0xFF53A13D), size: 24.r),
                  title: Text(
                    'اختر من المعرض',
                    style: GoogleFonts.cairo(),
                  ),
...
// Function to pick image
  ...
  // Function to get selected station ID
  int? getSelectedStationId() {
    return selectedStation != null ? stationsMap[selectedStation!] : null;
  }
                      ...
                      children: [
                        ...
                        _buildDropdownField(
                          ...
                          onChanged: (value) {
                            ...
                            // استدعاء الدالة لجلب المحطات
                            _fetchStationsFromServer();
                        ...
                        // Aidiya Type Dropdown
                        _buildDropdownField(
                          label: 'نوع العائدية',
                          value: selectedAidiya,
                          items: aidiyaTypes,
                          onChanged: (value) {
                            setState(() {
                              // امسح اختيار المحطة السابق عند تغيير العائدية
                              selectedStation = null;
                              selectedAidiya = value;
                              print(value);
                            });
                        ...
                        _buildDropdownField(
                          ...
                          items: stations,
                          isLoading: _isLoadingStations, // Add this line
                          onChanged: (value) {
                            setState(() {
                              selectedStation = value;
                              // ✅ السطر الأهم: جلب الـ ID من الـ Map وحفظه
                              selectedStationId = value != null ? stationsMap[value] : null;
                            });
                          },
                        ),
                        SizedBox(height: 16.h),
